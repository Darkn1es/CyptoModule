...
namespace CyptoModule.Models.Auxiliary
{
    public class Matrix
    {
        ...
        private int _module = 26;
        public int Module { get => this._module; }
        ...
        {
            var result = new Matrix(this.N, this.M, Module);
            result.ProcessFunctionOverData((i, j) => result[i, j] = this[j, i]);
            return result;
        }

        public Matrix(int m, int n, int module = 26)
        {
            this.m = m;
            this.n = n;
            this._module = module;
        ...
        public Matrix CreateInvertibleMatrix()
        {
            ...
            var result = new Matrix(M, M, Module);
            int inverseDet = inverseElement(determinant);
            ProcessFunctionOverData((i, j) =>
            {
                result[i, j] = calcMod(((i + j) % 2 == 1 ? -1 : 1) * CalculateMinor(i, j) * inverseDet);
            });
        ...
        private Matrix CreateMatrixWithoutRow(int row)
        {
            ...
            var result = new Matrix(this.M - 1, this.N, Module);
        ...
        private Matrix CreateMatrixWithoutColumn(int column)
        {
            ...
            var result = new Matrix(this.M, this.N - 1, Module);
        ...
        {
            var result = new Matrix(matrix.M, matrix.N, matrix.Module);
        ...
        public static Matrix operator *(Matrix matrix, Matrix matrix2)
        {
            ...
            var result = new Matrix(matrix.M, matrix2.N, matrix.Module);
        ...
        private int inverseElement(int a)
        {
            int x = 0;
            int y = 0;
            if (gcdExtended(a, Module, ref x, ref y) != 1)
            {
                throw new Exception("Обратный элемент не найден");
            }
