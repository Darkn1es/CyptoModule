...
namespace CyptoModule.Models.ModerCipher
{
    public class GOST28147Engine : IBlockCipher
	{
		...
		private static readonly IDictionary sBoxes = new Hashtable();
		...
		{
			if ( workingKey == null )
				throw new InvalidOperationException( "Ключи не сгенерировались" );

			Check.DataLength( input, inOff, BlockSize, "Входной блок слишком короткий" );
			Check.OutputLength( output, outOff, BlockSize, "Выходной блок слишком короткий" );
			...
			byte[] userKey )
		{
			...
			{
				throw new ArgumentException( "Ключ короткий" );
			}
			...
			int outOff )
		{
			...
			{
				for ( int j = 0; j < 8; j++ )  // 1-8 шаги
				{
					tmp = N1;
					N1 = N2 ^ Gost28147_mainStep( N1, workingKey[ j ] ); // CM2
					N2 = tmp;
				}
				for ( int k = 0; k < 3; k++ )  //9-31 шаги
				{
					for ( int j = 7; j >= 0; j-- )
					{
						if ( ( k == 2 ) && ( j == 0 ) )
						{
							break; // выйти с 32 шага
						}
						tmp = N1;
						N1 = N2 ^ Gost28147_mainStep( N1, workingKey[ j ] ); // CM2
						N2 = tmp;
			...
			N2 = N2 ^ Gost28147_mainStep( N1, workingKey[ 0 ] );  // 32 шаг (N1=N1)
